...
namespace QP {
    ...
    // Function to calculate forces towards mouse position and apply them to pendulums
    static void applyMouseForce(const Pendulum& p1, const Pendulum& p2, State& state, const Parameters& params, const QP::Vec2& mousePos) {
        double g = params.g;

        // Calculate force towards mouse position
        ...
        // Apply the force to the pendulums
        state.omega1 += force1 * 0.005f;
        state.omega2 += force2 * 0.005f;
    }

    static void equations_of_motion(const Pendulum& p1, const Pendulum& p2, const Parameters& params, const State& state, State& deriv) {
        double g = params.g;
    ...
    static void runge_kutta_step(const Pendulum& p1, const Pendulum& p2, const Parameters& params, State& state, double dt) {
